...
const STEPS = [
  ...
  { title: "Investigation Info", subtitle: "Police and IO details (optional)" },
...
export default function NewCase() {
  ...
  const validateStep = (stepIndex: number): boolean => {
    ...
    if (stepIndex === 0) {
      ...
      if (!form.accountMasked.trim())
        newErrors.accountMasked = "Masked account is required";
    ...
    if (stepIndex === 2) {
      ...
      if (!form.narrative.trim())
        newErrors.narrative = "Please describe what happened";
    }

    if (stepIndex === 4) {
      if (!form.consentGiven)
        newErrors.consentGiven = "You must agree to the terms";
    }
  ...
  const removeEvidenceFile = (id: string) => {
    updateField(
      "evidenceFiles",
      form.evidenceFiles.filter((f) => f.id !== id),
    );
  };

  const updateEvidenceMeta = (
    id: string,
    field: keyof EvidenceFile,
    value: string,
  ) => {
    updateField(
      "evidenceFiles",
    ...
    try {
      ...
      for (const ev of form.evidenceFiles) {
        await addEvidence({
          ...
          metadataAmount: ev.metadataAmount
            ? parseFloat(ev.metadataAmount)
            : undefined,
          ...
          <div className="flex items-center justify-between mb-2">
            {STEPS.map((s, i) => (
              <div key={i} className="flex items-center">
                ...
                {i < STEPS.length - 1 && (
                  <div
                    className={`hidden sm:block h-0.5 w-12 md:w-20 ml-2 ${
            ...
            <Card className="border-border/70">
              <CardContent className="p-6 md:p-8">
                ...
                <div className="mt-6 space-y-5">
                  ...
                  {step === 0 && (
                    <>
                      ...
                      <Field
                        ...
                        required
                        error={errors.accountMasked}
                        hint="Only last 4 digits, e.g. XXXX4321"
                      >
                        <Input
                          placeholder="XXXX4321"
                          value={form.accountMasked}
                          onChange={(e) =>
                            updateField("accountMasked", e.target.value)
                          }
                        />
                      </Field>
                      <Field
                        label="Freeze Type"
                        required
                        error={errors.freezeType}
                      >
                        <Select
                          value={form.freezeType}
                          onValueChange={(v) => updateField("freezeType", v)}
                        >
                          <SelectTrigger>
                      ...
                      <Field
                        label="Freeze Date"
                        required
                        error={errors.freezeDate}
                      >
                        <Input
                          type="date"
                          value={form.freezeDate}
                          onChange={(e) =>
                            updateField("freezeDate", e.target.value)
                          }
                        />
                      </Field>
                    </>
                  ...
                  {/* ── Step 1: Investigation Info ── */}
                  {step === 1 && (
                    <>
                      <p className="text-sm text-muted-foreground bg-muted/50 rounded-lg p-3">
                        All fields in this step are optional. Fill them in if
                      ...
                      <Field label="Police Station Name">
                        <Input
                          ...
                          onChange={(e) =>
                            updateField("policeStationName", e.target.value)
                          }
                        />
                      </Field>
                      ...
                      <Field label="Investigating Officer (IO) Name">
                        <Input
                          ...
                          value={form.ioName}
                          onChange={(e) =>
                            updateField("ioName", e.target.value)
                          }
                        />
                      </Field>
                      <Field label="IO Email">
                        <Input
                          type="email"
                          placeholder="io@police.gov.in"
                          value={form.ioEmail}
                          onChange={(e) =>
                            updateField("ioEmail", e.target.value)
                          }
                        />
                      </Field>
                      <Field label="FIR Number">
                        <Input
                          ...
                          onChange={(e) =>
                            updateField("firNumber", e.target.value)
                          }
                        />
                      </Field>
                    </>
                    ...
                    <>
                      ...
                      >
                        <Input
                          ...
                          onChange={(e) =>
                            updateField("transactionAmount", e.target.value)
                          }
                        />
                      </Field>
                      <Field
                      ...
                      >
                        <Textarea
                          placeholder="I received ₹15,000 from Rahul Enterprises on 15 Jan 2026 for a freelance web development project. I delivered the work on 10 Jan. I have no knowledge of any fraudulent activity related to this transaction..."
                          rows={5}
                          value={form.narrative}
                          onChange={(e) =>
                            updateField("narrative", e.target.value)
                          }
                        />
                      </Field>
                    </>
                    ...
                    <>
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Invoices, receipts, chat screenshots, delivery proofs,
                          etc.
                        </p>
                        <input
                          id="evidence-input"
                          ...
                          onChange={(e) => addEvidenceFile(e.target.files)}
                        />
                      </div>
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                      {(ev.file.size / 1024).toFixed(1)} KB
                                    </p>
                                  </div>
                                </div>
                                <button
                                  onClick={() => removeEvidenceFile(ev.id)}
                              ...
                              <div className="grid grid-cols-2 gap-3 mt-3">
                                <div>
                                  ...
                                  <Select
                                    value={ev.type}
                                    onValueChange={(v) =>
                                      updateEvidenceMeta(ev.id, "type", v)
                                    }
                                  >
                                ...
                                <div>
                                  ...
                                  <Input
                                    ...
                                    value={ev.metadataCounterpartyName}
                                    onChange={(e) =>
                                      updateEvidenceMeta(
                                        ev.id,
                                        "metadataCounterpartyName",
                      ...
                      {form.evidenceFiles.length === 0 && (
                        <p className="text-xs text-muted-foreground text-center mt-2">
                          You can also add evidence later from the case dashboard.
                        </p>
                      )}
                    </>
                  )}
                      ...
                      <div className="mt-6 rounded-xl border border-accent/20 bg-accent/5 p-5">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-2 text-xs leading-relaxed text-muted-foreground space-y-2">
                              <span className="block">
                                By checking this box, I confirm that:
                              </span>
                              <span className="block">
                                1. All information and evidence I have provided is true and accurate to the best of my knowledge.
                              </span>
                              <span className="block">
                                2. I understand that DIP does not certify innocence and cannot unfreeze accounts.
                              </span>
                              <span className="block">
                                3. I understand that knowingly false statements or fabricated evidence may have legal consequences.
                              </span>
                              <span className="block">
                                4. I remain solely responsible for the truthfulness of my submission.
                              </span>
                            </p>
                            <div className="mt-4 flex items-center gap-2">
                              <Checkbox
                                ...
                                onCheckedChange={(c) =>
                                  updateField("consentGiven", c === true)
                                }
                              />
                              <label
                                htmlFor="consent"
